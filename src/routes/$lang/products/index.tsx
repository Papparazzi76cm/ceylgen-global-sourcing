  ...
  return (
    <>
      <section className="container-page pt-16 md:pt-24 pb-8">
        ...
        <h1 className="mt-4 font-serif text-4xl md:text-6xl max-w-3xl">{t("catalog.title")}</h1>
        <p className="mt-5 max-w-2xl text-muted-foreground">{t("catalog.subtitle")}</p>
      </section>

      <section className="container-page pb-16">
        <div className="rounded-lg border border-border bg-card p-4 md:p-5 shadow-[var(--shadow-soft)]">
          <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr_1fr_1fr_1fr_auto] gap-3">
            ...
            <label className="flex items-center gap-2 h-11 px-3 rounded-sm border border-input bg-background text-sm cursor-pointer whitespace-nowrap">
              <input type="checkbox" checked={docOnly} onChange={(e) => setDocOnly(e.target.checked)} className="h-4 w-4 accent-primary" />
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <button onClick={reset} className="inline-flex items-center gap-1 hover:text-foreground">
              <X className="h-3 w-3" /> {t("catalog.reset")}
            </button>
          </div>
        </div>

        {filtered.length === 0 ? (
          <div className="mt-10 rounded-lg border border-dashed border-border p-16 text-center text-muted-foreground">
            {t("catalog.results.zero")}
          </div>
        ) : (
            ...
            {filtered.map((p, i) => {
              ...
              return (
                <Reveal key={p.slug} delay={i * 60}>
                  <article className="group h-full flex flex-col rounded-lg border border-border bg-card overflow-hidden shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-elevated)] hover:-translate-y-0.5 transition-all">
                    <div className="relative aspect-[4/3] overflow-hidden bg-graphite">
                      ...
                      <span className={cn("absolute top-3 left-3 rounded-sm bg-background/90 backdrop-blur px-2 py-1 text-[10px] font-semibold uppercase tracking-wider", accentClasses[c.accent].text)}>
                        {categoryName(lang, c)}
                      </span>
                      {p.hasTechnicalSheet && (
                        <span className="absolute top-3 right-3 flex items-center gap-1 rounded-sm bg-champagne/95 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-graphite">
                          <FileText className="h-3 w-3" /> TDS
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col flex-1 p-5">
                      ...
                      <div className="mt-4 flex flex-wrap gap-1.5">
                        {info.applications.slice(0, 3).map((a) => (
                          <span key={a} className="text-[10px] px-2 py-0.5 rounded-full border border-border text-muted-foreground">{a}</span>
                        ))}
                      </div>
                      <div className="mt-5 flex items-center justify-between pt-4 border-t border-border">
                        <Link to="/$lang/products/$slug" params={{ lang, slug: p.slug }}
                          ...
                          {t("catalog.card.view")} <ArrowRight className="h-3.5 w-3.5" />
                        </Link>
                        <Link to="/$lang/contact" params={{ lang }} className="text-xs text-muted-foreground hover:text-foreground">
  ...
  return (
    <div>
      <select value={value} onChange={(e) => onChange(e.target.value)} aria-label={label}
        className="w-full h-11 rounded-sm border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
